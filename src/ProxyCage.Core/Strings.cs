...
public static class Strings
{
    ...
    private static readonly Dictionary<string, Pair> Table = new(StringComparer.Ordinal)
    {
        ...
        ["doc_ping_btn"] = new("Проверить связь", "Test connection"),
        ["job_ping_test"] = new("Проверяю связь (пинг)", "Testing connection (ping)"),
        ["ping_stage_direct"] = new("Этап 1: Прямое подключение (без прокси)", "Stage 1: Direct connection (no proxy)"),
        ["ping_stage_direct_n"] = new("Этап 1: Прямой пинг {0} из {1}", "Stage 1: Direct ping {0} of {1}"),
        ["ping_stage_proxy"] = new("Этап 2: Подключение через ноду (прокси)", "Stage 2: Connection via node (proxy)"),
        ["ping_stage_proxy_n"] = new("Этап 2: Пинг через прокси {0} из {1}", "Stage 2: Proxy ping {0} of {1}"),
        ["ping_card_title"] = new("Проверка связи с интернетом", "Internet connection test"),
        ["ping_target_label"] = new("Цель проверки", "Test target"),
        ["ping_direct_title"] = new("1 этап: Прямой интернет (без прокси)", "Stage 1: Direct internet (no proxy)"),
        ["ping_proxy_title"] = new("2 этап: Через ноду (прокси)", "Stage 2: Via proxy node"),
        ["ping_result_stat"] = new("{0} из {1} успешно ({2}%), отклик {3} мс", "{0} of {1} successful ({2}%), latency {3} ms"),
        ["ping_result_stat_fail"] = new("0 из {0} успешно (0%), ошибка: {1}", "0 of {0} successful (0%), error: {1}"),
        ...
        ["app_pick_no_user"] = new(
            "Не удалось открыть выбор файла в вашем сеансе. Укажите путь вручную в поле слева — или сверните RDP и откройте панель снова.",
            "Could not open the file picker in your session. Type the path in the field on the left, or reconnect via RDP and try again."),
        ["app_pick_failed"] = new(
        ...
        ["apps_browser_live_title"] = new(
            "Браузер уже был запущен",
        ...
        ["bounced_network"] = new(
            "Сбросил сетевые процессы ({0}): {1}. Новые запросы пойдут через туннель.",
            "Reset network processes ({0}): {1}. New requests will go through the tunnel."),
        ...
        ["stage_bounce_apps"] = new(
            "Сбрасываю старые соединения браузера",
            "Resetting old browser connections"),
